...
$data['module'] = $module++;
		if ($data['categories']) {
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/featured_categories.tpl')) {
				return $this->load->view($this->config->get('config_template') . '/template/module/featured_categories.tpl', $data);
			} else {
				return $this->load->view('default/template/module/featured_categories.tpl', $data);
			}
		}
	}
}