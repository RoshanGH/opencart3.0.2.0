	...
	********************************/
	...
	var openOldImageDialog = function(title,onInsert){
		...
		$('#sds_thumb').remove();
			...
			$.ajax({
				url: 'index.php?route=common/filemanager&user_token=' + getURLVar('user_token') + '&target=sdstest_target&thumb=sdstest_thumb&directory=revslider_media_folder',
				dataType: 'html',
...
function image_upload(field, thumb) {
	...
	$('body').prepend('<div id="dialog" style="padding: 3px 0px 0px 0px;"><iframe src="index.php?route=common/filemanager&user_token='+sds_rev_oc_token+'&field=' + encodeURIComponent(field) + '" style="padding:0; margin: 0; display: block; width: 100%; height: 100%;" frameborder="no" scrolling="auto"></iframe></div>');
	...
	$('#dialog').dialog({
		...
		close: function (event, ui) {
			if ($('#' + field).attr('value')) {
				$.ajax({
					url: 'index.php?route=common/filemanager/image&user_token='+sds_rev_oc_token+'&image=' + encodeURIComponent($('#' + field).attr('value')),
					dataType: 'text',
