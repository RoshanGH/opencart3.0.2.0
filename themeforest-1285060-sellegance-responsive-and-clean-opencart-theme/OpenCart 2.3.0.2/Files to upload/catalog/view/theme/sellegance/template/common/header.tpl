...
<head>
	...
	<?php
	$layout = $sellegance_responsive_layout;
	...
	<?php 
	if (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] != 'off') $path_image = $config_ssl . 'image/';
	...
	<?php if($sellegance_full_background == 1) { ?>
		...
		jQuery(function($){
			$.supersized({
				slides:[ {image: '<?php echo $path_image . $sellegance_custom_image ?>'} ]
			});
		});
		</script>
...
<div class="topcanvas">
	...
	<header id="header">
		...
		<div class="container">
			...
			<div class="header_inner <?php echo ($sellegance_searchbox_regular==1) ? 'searchbox-regular' : 'searchbox-icon'; ?> <?php echo $sellegance_logo_position;?>logo">

				<div class="row">

					<div id="logo" class="col-sm-4 col-xs-6 <?php echo ($sellegance_logo_position=='center') ? 'col-sm-push-4 col-xs-push-3' : ''; ?>">
						<?php if ($logo) { ?>
					...
					<div class="col-sm-4 col-xs-3 leftbox <?php echo ($sellegance_logo_position=='center') ? 'col-sm-pull-4 col-xs-pull-6' : 'col-sm-pull-0 hidden-xs'; ?>">

						<?php if(isset($sellegance_header_info_text) && strip_tags(html_entity_decode(implode($sellegance_header_info_text)))!='' && isset($sellegance_header_info_text[$language_id])) { ?>
						<div class="header-info owl-carousel <?php echo ($sellegance_logo_position=='center') ? 'hidden-sm' : ''; ?> hidden-xs">
							<?php echo html_entity_decode($sellegance_header_info_text[$language_id], ENT_QUOTES, 'UTF-8');?>
						</div>
						...
						<?php if ($sellegance_logo_position=='center') { ?>
					...
					<div class="col-sm-4 <?php echo ($sellegance_logo_position=='center') ? 'col-xs-3' : 'col-xs-6'; ?> rightbox search-<?php echo $sellegance_searchbox_regular; ?>">

						<?php if ($sellegance_logo_position=='left') { ?>
							<button type="button" class="btn btn-menu visible-sm-inline visible-xs-inline" onclick="$('.topcanvas').addClass('canvas-slid');" data-toggle="offcanvas" data-target="#mobmenu" data-disable-scrolling="true" data-modal="true" ><i class="fa fa-bars"></i></button>
						<?php } ?>
	...
	<?php 
	($sellegance_fullwidth_dropdown_menu == 1) ? $fulldrop='fullwidth' : $fulldrop='';
	$navstyle=$sellegance_menu_style;
	?>
