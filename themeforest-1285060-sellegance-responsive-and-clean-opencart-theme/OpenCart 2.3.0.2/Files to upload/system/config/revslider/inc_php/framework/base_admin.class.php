		...
		public static function getViewUrl($viewName,$urlParams="")
		{
			$oc_token = sds_get_oc_token();
                    ...
                    if(isset($_GET['returnurl'])){
                        ...
                        $link = $links[0];
                        $link .= "&route=extension/module/revslideropencart&view={$viewName}&token={$oc_token}";
                        $link .= "&{$urlParams}";   
                    }
                    else{
                        $link = admin_url("index.php?route=extension/module/revslideropencart&token=".$oc_token."&page=".self::$dir_plugin.$params);
                    }
                  return($link);
		}
